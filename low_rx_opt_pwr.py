...
def ont_detail(e9, port):
    ont = get(
        f"https://10.20.7.10:18443/rest/v1/performance/device/{e9}/ont/{port}/status",
...
def affected(e9, instid, port, names=set()):
    ...
    for i in r[1:-1]:
        get_phone = get(
            f"https://10.20.7.10:18443/rest/v1/ems/subscriber/device/{e9}/port/{port}%2Fx1",
            auth=("admin", "Thesearethetimes!"),
            verify=False,
        )
        r1 = get_phone.json()
...
def clean_up(e9, instid, cnct):
    ...
    path.append("/home/derrick/Documents/CVEC_Stuff/low-rx-pwr/")
    cnct.send_command_timing(f"manual acknowledge instance-id {instid}")
    run(
        f"mv {e9}_{date.today()}.txt {path[-1]}/{e9}/{e9}_{date.today()}.txt",
        shell=True,
    )


def process_alarm(tbl, ont_ids=set()):
    alarms = tbl.split("\n")
    for alarm in alarms:
        data = alarm.split()
        if not len(data) >= 13:
